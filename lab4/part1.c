...
void *producer(void *ptr) {
  ...
  for (size_t i = 0; i < strlen(g_prod_str); i++) {
    ...
    g_buffer.buf[g_buffer.tail++ % 32] = g_prod_str[i];
...
void *consumer(void *ptr) {
  ...
  for (size_t i = 0; i < strlen(g_prod_str); i++) {
    ...
    pthread_mutex_lock(&g_buffer_mutex);
    char c = g_buffer.buf[g_buffer.head++ % 32];
...
int main(int argc, char **argv) {
  ...
  int producers_total = atoi(argv[1]);
  int consumers_total = atoi(argv[2]);
  pthread_t consumers[consumers_total];
  pthread_t producers[producers_total];

  for (int i = 0; i < producers_total; i++) {
    int *int_ptr = 0;
    int_ptr = (int *)malloc(sizeof(int));
    *int_ptr = i;
    pthread_create(&producers[i], NULL, producer, (void *) int_ptr);
  }
  for (int i = 0; i < consumers_total; i++) {
    int *int_ptr = 0;
    int_ptr = (int *)malloc(sizeof(int));
    *int_ptr = i;
    pthread_create(&consumers[i], NULL, consumer, (void *) int_ptr);
  }
  for (int i = 0; i < producers_total; i++) {
